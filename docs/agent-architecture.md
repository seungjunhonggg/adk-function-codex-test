...
- 사용자 메시지 진입점
- 라우팅/커맨드 결정, 입력·변경 파싱
- SequentialAgent 실행 및 최종 응답 스트리밍

### 2) SequentialAgent (시뮬레이션)
- StageAgent 1-1 ~ 1-7 순차 실행
- dirty 단계만 실행, 나머지는 스킵
- gap 발생 시 즉시 중단

### 3) StageAgent (단계 실행)
- 각 단계는 독립 실행 함수만 가진다
- before_agent_callback으로 dirty/중단 여부 확인
- 결과는 tables/charts/stage_notes/gap로 반환
