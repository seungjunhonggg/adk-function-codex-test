# /api/chat 워크플로우 (v2, Google ADK)

## 목적
- 캐주얼 대화와 시뮬레이션 요청을 분기한다.
- 시뮬레이션은 1-1~1-7 단계까지만 수행한다.
- 브리핑 생성(장문 요약)은 사용하지 않는다.
...
## 스트리밍 응답 (SSE)
- `/api/chat/stream`은 text/event-stream으로 텍스트 이벤트만 보낸다.
- 이벤트 이름은 `delta`이며 payload는 `{ "text": "..." }` 형식이다.
