...
## 표기 규칙
- 데이터베이스나 외부 소스에서 컬럼명을 가져올 때는 가능한 한 한글 컬럼명으로 변환해 사용한다 (텍스트/표/그래프 모두).

*** 1) 생각하는 규칙
- 사용자가 현재 코드에서 엣지케이스를 말하며 개선을 요청하면 딱 그 예시만 해결하려고 하지 말고, 그 예시가 왜 어떤 엣지케이스에서 비롯됐는지 파악하고, 그 근간을 해결할수 있는 해결책을 고민해. 특히 메모리 구조와 멀티에이전트간 구조 관점에서 더 매끄럽게 연결하기 위한 관점을 생각해.
...
** 코드 너무 안정성만을 강요해서 복잡하게 짜지 말것. 그냥 값 받은걸 기준으로 쉽게쉽게 짜야함. 저년차 개발자들도 쉽게 이해할수 있는 코드여야함.. 한달 개발한 개발자도 알기쉽게 최대한 직관적으로 작성. 절대 검증이나 방어적으로 짜지 말것. **
